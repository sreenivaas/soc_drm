							...
							const CFftPlans& FftPlans)
{
	...
	_FCOMPLEX* pFftwComplexIn=new _FCOMPLEX[n];
	_FCOMPLEX* pFftwComplexOut=new _FCOMPLEX[n];



	CMatlibVector<FComplex>	cvReturn(n, VTY_TEMP);
	CMatlibVector<CComplex>	FP_cvReturn(n, VTY_TEMP);
	...
	{
        //Religion change
        pFftwComplexIn[i]= (FComplex) cvI[i];
	}
	...
	for (i = 0; i < n; i++)
	{
//        complex<_FREAL>::iterator it1; 
//        complex<_REAL>::iterator  it2;
//
//        for ( it1=pFftwComplexIn[i].begin(), it2=FP_cvReturn[i].begin() ; it1 < pFftwComplexIn[i].end(); it1++, it2++ )
//            *it2 = (_REAL) *it1;
        FP_cvReturn[i].real() = (double)pFftwComplexIn[i].real();
        FP_cvReturn[i].imag() = (double)pFftwComplexIn[i].imag();
	}
	return FP_cvReturn;
