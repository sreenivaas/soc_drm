		...
		const CFftPlans& FftPlans)
{
	...
	_FREAL *pFftwRealIn = new _FREAL[iSizeI];
	_FCOMPLEX* pFftwComplexOut = new _FCOMPLEX[iShortLength+1];
		...
		const CFftPlans& FftPlans)
{
	...
	CMatlibVector<FXP> fvReturn(iLongLength, VTY_TEMP);
	_FCOMPLEX* pFftwComplexIn = new _FCOMPLEX[cvI.GetSize()];
	_FREAL* pFftwRealOut=new _FREAL[iLongLength];

	for(i=0; i<cvI.GetSize(); i++)
		pFftwComplexIn[i]=_FCOMPLEX(cvI[i].real(), cvI[i].imag());
	...
	const CFix scale = (CFix) 1.0 / iLongLength;
	for (i = 0; i < iLongLength; i++) 
		fvReturn[i] = pFftwRealOut[i] * scale;
